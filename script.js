...
// }<script>
  ...
  function performSearch() {
    var searchTerm = document.getElementById('searchInput').value.toLowerCase(); // Get the search input and convert to lowercase for case-insensitive search

    // Loop through elements you want to search (e.g., portfolio items)
    var elementsToSearch = "#portfolio_section"('portfolio');

    for (var i = 0; i < elementsToSearch.length; i++) {
      var element = elementsToSearch[i];
      var text = element.innerText.toLowerCase(); // Get the text content of the element
  ...
  // Attach the search function to the search input's "input" event
